...
public class UpdateTeapawd extends JFrame {
    private JTextField textField;
    private JTextField textField_1;
    public UpdateTeapawd() {
        ...
        button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (textField.getText().isEmpty()) {
                    ...
                    JOptionPane.showMessageDialog(null, "账号仅由数字组成，您输入的账号形式有误!\n请重新输入", "提示消息", JOptionPane.ERROR_MESSAGE);
                } else if (!MysqlConn.querytea(textField.getText())) {
                    JOptionPane.showMessageDialog(null, "您输入的账号有误或该账号不存在!\n请重新输入", "提示消息", JOptionPane.ERROR_MESSAGE);
                } else if (textField_1.getText().isEmpty()) {
                    JOptionPane.showMessageDialog(null, "请输入密码!", "提示消息", JOptionPane.WARNING_MESSAGE);
                } else {
                    MysqlConn.teaid = textField.getText();
                    String s = textField_1.getText();
                    MysqlConn.updateteapawd(MysqlConn.teaid,s);
