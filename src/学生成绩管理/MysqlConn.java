...
public class MysqlConn {
    ...
    static final String DB_URL = "jdbc:mysql://localhost:3306/SCIS?characterEncoding=utf-8";

    //数据库登录名及密码
    static final String USER = "root";
    static final String PASS = "lyc238";
    ...
    public static boolean querycname(String cname) {
        //
        try {
            ps = conn.prepareStatement("select * from 学生 where Sname = ?");
            // 给?赋值(可防止SQL注入漏洞问题)，不要直接使用拼接的方式
    ...
    public static void insertstu() {
        try {
            ps = conn.prepareStatement("insert into 学生 values ('" + stuid + "','" + stuid + "','" + stuname + "','" + stusex + "','" + stubirth + "','" + stutel + "','" + stuacademy + "','" + stuclass + "','" + stuadd + "')");
            ps.executeUpdate();
    ...
    //修改学生密码
    public static void updatestupawd(String stuid2, String newpawd) {
        try {
            ps = conn.prepareStatement("update 学生 set Password = ? where ID = ? ");
            ps.setString(1, newpawd);
            ps.setString(2, stuid2);
            ps.executeUpdate();
            JOptionPane.showMessageDialog(null, "修改成功！", "提示消息", JOptionPane.WARNING_MESSAGE);
        } catch (Exception e1) {
            // TODO Auto-generated catch block
            JOptionPane.showMessageDialog(null, "密码修改失败!", "提示消息", JOptionPane.ERROR_MESSAGE);
            e1.printStackTrace();
        }
    ...
    public static void inserttea() {
        // TODO Auto-generated method stub
        try {
            ps = conn.prepareStatement("insert into 教师 values ('" + teaid + "','" + teaid + "','" + teaname + "','" + teabirth + "','" + protitle + "','" + cno + "')");
    ...
    public static void updateteapawd(String teaid2, String newpawd) {
        try {
            ps = conn.prepareStatement("update 教师 set Password = ? where ID = ?");
            ps.setString(1, newpawd);
            ps.setString(2, teaid2);
            ps.executeUpdate();
            JOptionPane.showMessageDialog(null, "修改成功！", "提示消息", JOptionPane.WARNING_MESSAGE);
        } catch (Exception e1) {
            // TODO Auto-generated catch block
    ...
    public static void query_1(String stuid,String cname2) {
        try {
            ps = conn.prepareStatement("select * from `学生`,`课程`,`成绩` where ID = ? and Cname =? and `课程`.Cno = `成绩`.Cno and `学生`.ID = `成绩`.Sno  ");
    ...
    public static void query_2(String stuclass,String cname2) {
        try {
            ps = conn.prepareStatement("select * from `学生`,`课程`,`成绩` where Sclass = ? and Cname = ? and `课程`.Cno = `成绩`.Cno and `学生`.ID = `成绩`.Sno");
    ...
    public static boolean queryclass(String stuclass) {
        //
        try {
            ps = conn.prepareStatement("select * from 学生 where Sclass= ?");
            // 给?赋值(可防止SQL注入漏洞问题)，不要直接使用拼接的方式
            ps.setString(1, stuclass);
            // ResultSet结果集,大家可以把ResultSet理解成返回一张表行的结果集
            rs = ps.executeQuery();
    ...
    public static void query_3(String stuname,String cname2) {
        try {
            ps = conn.prepareStatement("select * from `学生`,`课程`,`成绩` where Sname = ? and Cname = ? and `课程`.Cno = `成绩`.Cno and `学生`.ID = `成绩`.Sno");
